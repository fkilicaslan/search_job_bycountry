...
This system has been used to evaluate hundreds of job offers, generate tailored CVs, and land senior roles across multiple career searches. The archetypes, scoring logic, negotiation scripts, and proof point structure are designed to adapt to any candidate — not just the person who originally configured the repo.
...
| `setup.mjs` | First-run wizard — parse CV → Claude extract → prompts → write profile.yml + cv.md + portals.yml |
...
**Fastest path:** If the user hasn't set up the system yet, suggest running the setup wizard first:
```
node setup.mjs --cv path/to/your_cv.pdf --country Germany
```
This single command parses their CV, calls Claude to extract structured data, asks a handful of targeted questions, and writes `config/profile.yml`, `cv.md`, and `portals.yml` in one pass. After it completes, skip to Step 6 (Ready).

**If the wizard isn't suitable** (no CV file, or the user prefers manual setup), follow Steps 1–5 below instead.
...
| Says "quick eval", "quick score", "just give me a number" | `quick-eval` |
| Asks to compare offers / "which should I apply to" | `compare` |
| Wants LinkedIn outreach / cold email / recruiter message | `outreach` |
...
Rich context for evaluations, interview prep, and cover letters. Read proactively when relevant:

| File | When to read |
|------|-------------|
| `data/corpus/narrative/swot.md` | Any evaluation — use Strengths for proof points, Weaknesses for gap framing, ⚠ German flag for language requirements |
| `data/corpus/narrative/interview-stories.md` | Block F of evaluations, cover letters, application form answers |
| `data/corpus/narrative/leadership-philosophy.md` | Leadership-focused roles, "why should we hire you" answers, outreach proof points |

### Scoring Reference

- `references/scoring-rubric.md` — weighted dimensions, role-type adjustments, hard-cap rules, German language flag. Read before scoring any evaluation.
